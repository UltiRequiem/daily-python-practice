...
In this example, the first element can be split into two words: hello and cat because both of 
...
#str_arr = ["hellocat", "apple,bat,cat,goodbye,hello,yellow,why"]
str_arr =["baseball", "a,all,b,ball,bas,base,cat,code,d,e,quit,z"]


def word_split(arr):
    word_to_split = arr[0]
    possible_words = arr[1].split(",")

    separated_words = []

    for item in possible_words:
        if item in word_to_split:
            separated_words.append(item)

    print(separated_words[1], ",", separated_words[0])
